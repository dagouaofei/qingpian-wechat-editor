...
- 缺少统一的产品全景：产品愿景、用户、旅程、模块树、功能目录、Story Map、Release/Sprint 之间没有单一映射入口。
...
- Story Map 当前偏 Release 1 / Sprint 6 视角，不能覆盖 Release 2 及后续产品全景。
...
- 当前仓库已存在 “S12-STORY-001 WeChat Compatibility Spec Recalibration” 占位项；本轮用户指令将 S12-STORY-001 正式用于 Product Governance 审计，形成 ID 冲突。该冲突不应靠重编号历史记录解决，需在 Sprint 12 后续治理 Story 中作正式映射决策。
...
| `docs/product/user-story-map.md` | Release 1 用户路径与 P0 US  | 部分             | product-backlog、sprint-backlog    | 仅覆盖 R1/S6 主体        | 升级为多 Release Story Map                                    |
...
| Story Map       | 存在         | `user-story-map.md`                                   | 中     | 不是完整多 Release story map                     |
