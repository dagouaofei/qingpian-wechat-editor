...
- **Story 顺序执行** 清晰：002→003→004→005→006→007→008→009 单 Story 分支 + 验收 + merge 节奏稳定
...
- **Consistency Audit**（009）在 Closeout 前捕获全局索引滞后问题
...
- **`.cursor/rules/` 未同步** S12 新治理事实源（登记为 **P1-S12-001** follow-up）
...
2. Closeout 前强制跑 **consistency-audit**（本 Sprint 模式可复用）
...
- Operating Model 应成为 `.cursor/rules/` 对齐的**单一上游**（**P1-S12-001**）
- Release 2 候选规划与 **R2 启动** 之间须保留显式 PO Approved 闸门（**P1-S12-003**）
...
- 验收轮与执行轮分离清晰
- Closeout Story 指令引用 `consistency-audit.md` 结论模板有效
...
1. R2 启动前：PO 明确 **Release 2 Approved**（**P1-S12-003** · 独立于候选规划）
2. R2-Sprint-01 须独立 Sprint Planning；不得直接执行 `releases/release-2/backlog.md` Must 项
3. Release 1 Closeout 与 Sprint 12 Closeout **分开决策**（**P1-S12-002**）
4. `.cursor/rules/` 对齐可作为 Sprint 12 后首个 chore（**P1-S12-001**）
