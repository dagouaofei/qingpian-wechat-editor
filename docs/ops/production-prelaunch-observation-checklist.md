...
| # | 检查项 | 期望 | ☐ |
|---|--------|------|---|
| O72-1 | T+24h 全部项复测 | PASS | |
| O72-2 | TLS 证书有效期 | >14 天（observe 自动检查） | |
| O72-3 | Certbot 续期任务 | 存在且未报错 | |
| O72-4 | 回滚路径仍可用 | 文档 + 脚本未漂移 | |
| O72-5 | deferred 项未误执行 | 无 governance apply · 无 P1-S11-001 变更 | |
